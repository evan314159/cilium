	       ...
	       bool from_tunnel, __u32 cluster_id)
{
	 ...
	 *
	 * This is strictly the inverse of should_redirect_peer(): a redirect_peer()
	 * bypasses cil_to_container, so policy must be enforced here, while a plain
	 * redirect() into the lxc device runs cil_to_container at that device's
	 * egress, so enforcing here as well would evaluate the packet twice. Keep
	 * the two in sync - a mismatch either drops the packet on a second policy
	 * pass with the wrong source identity, or re-runs the ingress path on a
	 * packet that lb{4,6}_rev_nat() already reverse NATed.
	 */
